window.addEventListener("load", function () {
    ...
    function crypterMessage() {
        ...
        if (controle) {
            ...
            // insersion de l'entête indiquant la taille de l'image
            //traduit la longeur du message en code ascii (hexadécimal)
            var len_bin = "",
                len_bin = "";

            len_bin = msg_len.toString(2) + ""; // + "" permet d'avoir une chaine de caractère (en binaire)



            while (1) {
                //si la longueur du caractère en bin fait 15, sors de la boucle
                if (len_bin.length == 15) {
                    break;
                    //sinon rajoute assez de 0 pour arriver à des lettres codés sur 2 octet - 1 bit
                } else {
                    len_bin = "0" + len_bin;
                }
            }
            var msg = len_bin + msg_bin;

            //insertion du message dans l'image

            // indique que le message caché est une chaine de caractères : le premier bit est alors pair
            for (var k = 0; k < 4; k++) {
                if (tableauPixel[k] % 2 == 0) {
                    // ne rien faire (0)
                } else {
            ...
            // insère le message : entête + message caché
            // pour chaque pixel 
            var i = 0;
            for (var k = 4; k < tableauPixel.length; k += 4) {



                //les différentes possibilités
                if (msg[i] == 0 && tableauPixel[k] % 2 == 0) {
                    //ne rien faire, la composante couleur est déjà paire 
                }

                //si bit = 0 et composante impaire:
                else if (msg[i] == 0 && tableauPixel[k] % 2 == 1) {
                ...
                else if (msg[i] == 1 && tableauPixel[k] % 2 == 0) { // % = modulo (reste de la division euclidienne)
                    //si coueleur noir (0) on ne peut que ajouter 1
                    if (tableauPixel[k] == 0) {
                        tableauPixel[k] = tableauPixel[k] + 1;
                        //sinon on retire 1 (car si couleur blanche (255) il y a depassement de capacité )
                ...
                } else if (msg[k] == 1 && tableauPixel[k] % 2 == 1) {
            ...
            var long1 = tableauPixel1.length; //longueur du tableau
            var messDecrypt1 = [];
            for (var z = 0; z < long1; z += 4) {
                if (tableauPixel1[z] % 2 == 0) {
                    messDecrypt1.push(0);
                } else {
                    messDecrypt1.push(1)
                }
    ...
    //les différentes possibilités
                ...
                } else if (msg_bin[i] == 1 && tableauPixel[k]%2 == 1) {
                    //ne rien faire, composante couleur déjà impaire
                }
            ...
            console.log(tableauPixel1);
            console.log(tableauPixel);
            var long1 = tableauPixel.length; //longueur du tableau
            var messDecrypt1 = [];
            for (var z=0 ; z < long1; z += 4) {
                if (tableauPixel1[z]%2 == 0) {
                    messDecrypt1.push(0);
            ...
            console.log("image = ",  messDecrypt1);
            var long = tableauPixel.length; //longueur du tableau
            var messDecrypt = [];
            for (var z=0 ; z < long; z += 4) {
                if (tableauPixel[z]%2 == 0) {
